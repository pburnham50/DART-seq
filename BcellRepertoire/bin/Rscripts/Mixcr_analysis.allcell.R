...
fastq = data.frame(fread(paste0("results/",sam,"/",sam,".tagged.allcell.reads"),sep = ":",header = F))
fastq2 = data.frame("SeqNum" = c(1:nrow(fastq)),"Cell" = fastq$V10, "UMI" = fastq$V13, "fullID" = paste0(fastq$V10,fastq$V13))

mixcr.results = data.frame(fread(paste0("results/",sam,"/",sam,".tagged.allcell.alignments.txt"), header = T, sep  = "\t"))
...
allcell.list = as.character(as.matrix(fread(paste0("results/",sam,"/",sam,".allcell.barcodes.txt"),header = F)))
...
composite.df = data.frame(matrix(unlist(lapply(allcell.list, function(x) heavy_light_presence(x))),ncol = 4, byrow = T))
...
write.table(all.data,paste0("results/",sam,"/",sam,".allcell.IGstats.txt"),sep="\t", quote = F, row.names = F, col.names = T)
write.table(region_gen("V"),paste0("results/",sam,"/",sam,".allcell.Vgenecount.txt"),sep="\t", quote = F, row.names = F, col.names = T)
write.table(region_gen("D"),paste0("results/",sam,"/",sam,".allcell.Dgenecount.txt"),sep="\t", quote = F, row.names = F, col.names = T)
write.table(region_gen("J"),paste0("results/",sam,"/",sam,".allcell.Jgenecount.txt"),sep="\t", quote = F, row.names = F, col.names = T)
write.table(region_gen("C"),paste0("results/",sam,"/",sam,".allcell.Cgenecount.txt"),sep="\t", quote = F, row.names = F, col.names = T)
...
write.table(vdjc_tab,paste0("results/",sam,"/",sam,".allcell.chains.txt"),sep="\t", quote = F, row.names = F, col.names = T)
